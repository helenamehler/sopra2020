...
public class ResidualGraphImpl<V> implements ResidualGraph<V>, ExerciseSubmission {
	...
	public ResidualGraphImpl(FlowGraph<V> flowGraph) {
		...
		for (V node : flowGraph.getNodes()) {
			// converts flow edge into residual edge and puts it with reverse edge into the
			// graph
			for (FlowEdge<V> flowEdge : flowGraph.edgesFrom(node)) {
				// checks if edges are already in graph
				if (!contains(flowEdge)) {
					// new edges
					ResidualEdgeImpl<V> edge1 = new ResidualEdgeImpl<V>(flowEdge.getStart(), flowEdge.getEnd(),
							flowEdge.getCapacity());
					ResidualEdgeImpl<V> edge2 = new ResidualEdgeImpl<V>(flowEdge.getEnd(), flowEdge.getStart(),
							flowEdge.getCapacity());
	 ...
	 * @param end   - end node
	 * @return the residual edge between start and end node or null if the edge
	 *         doesn't exist
	...
	public ResidualEdge<V> getEdge(V start, V end) {
		...
		// checking all elements from the arraylist of the start node if the edge is
		// there
	...
	/**
	 * checks if graph contains a given node, probably better that contains method
	 * of collection because that method only compares objects(class)
	 * 
